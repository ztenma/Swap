...
class Grid(object):
	...
	"""def testCombo(self, x, y): # TODO
		...
		for i in range(self.width):
			print('({0},{1}): {2} c{3}'.format(i, y, self[i][y], combo), end='\n')
			if self[i][y] == ref:
				combo += 1
			if i == self.width - 1 or self[i+1][y] != ref:
				if combo >= 3:
					print('blocks:', [(k, y) for k in range(i+1-combo, i+1)])
					toBeDeleted.update([(k, y) for k in range(i+1-combo, i+1)])
				break
		...
		for j in range(self.height):
			print('({0},{1}): {2} c{3}'.format(x, j, self[x][j], combo), end='\n')
			if self[x][j] == ref:
				combo += 1
			if j == self.height - 1 or self[x][j+1] != ref:
				if combo >= 3:
					print('blocks:', [(x, k) for k in range(j+1-combo, j+1)])
					toBeDeleted.update([(x, k) for k in range(j+1-combo, j+1)])
				break
	...
	def testComboAll(self):
		...
		for x in range(self.width):
			for y in range(self.height):
				...
				#print('({},{}) ref {} ycombo {}'.format(x, y, ref, combo))
				if y >= 1 and ref == self[x][y-1]:
					combo += 1
				if ref != self[x][y-1]:
					if combo >= 3:
						#print('blocks:', [(x, j) for j in range(y-combo, y)])
						toBeDeleted.update([(x, j) for j in range(y-combo, y)])
						#print('ycombo:', combo)
					combo = 1
				if y == self.height - 1 and combo >= 3:
					#print('blocks:', [(x, j) for j in range(y-combo+1, y+1)])
					toBeDeleted.update([(x, j) for j in range(y-combo+1, y+1)])
		...
		for y in range(self.height):
			for x in range(self.width):
				...
				#print('({},{}) ref {} xcombo {}'.format(x, y, ref, combo))
				if x >= 1 and ref == self[x-1][y]:
					combo += 1
				if self[x-1][y] != ref:
					if combo >= 3:
						#print('blocks:', [(i, y) for i in range(x-combo, x)])
						toBeDeleted.update([(i, y) for i in range(x-combo, x)])
						#print('xcombo:', combo)
					combo = 1
				if x == self.width - 1 and combo >= 3:
					#print('blocks:', [(i, y) for i in range(x-combo+1, x+1)])
					toBeDeleted.update([(i, y) for i in range(x-combo+1, x+1)])
			combo = 1
...
def test2():
	_grid = [[randrange(5) for _ in range(20)] for _ in range(18)]
	grid = Grid(data=_grid, nbSymbols=5)
	#grid = Grid(40, 20, 6)
	print('init\n' + str(grid))
	input()
	...
	while combo:
		...
		combo, comboPos = grid.testComboAll()
		if combo:
			score += len(comboPos)
			for pos in comboPos: # Remove combos
				grid[pos] = 0
			print('testCombo1All:', combo, len(combo), '\n' + str(grid))
			time.sleep(.5)
